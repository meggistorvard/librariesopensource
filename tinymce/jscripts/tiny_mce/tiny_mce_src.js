(function(win) {
	...
	var tinymce = {
		...
		_init : function() {
			...
			// TinyMCE .NET webcontrol might be configuration the values for TinyMCE
...
(function() {
	...
	tinymce.create('tinymce.util.JSONRequest', {
		...
		send : function(o) {
			...
			o = extend(this.configurations, o);
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.DOMUtils', {
		...
		DOMUtils : function(d, s) {
			...
			t.configurations = s = tinymce.extend({
				keep_values : false,
				hex_colors : 1,
				process_html : 1
			}, s);
		...
		getParents : function(n, f, r, c) {
			var t = this, na, se = t.configurations, o = [];
		...
		select : function(pa, s) {
			...
			return tinymce.dom.Sizzle(pa, t.get(s) || t.get(t.configurations.root_element) || t.doc, []);
		},
		...
		setStyle : function(n, na, v) {
			...
			return t.run(n, function(e) {
				...
				if (t.configurations.update_styles)
		...
		setStyles : function(e, o) {
			var t = this, s = t.configurations, ol;
		...
		getAttrib : function(e, n, dv) {
			...
			if (n === 'style') {
				...
				if (v) {
					...
					if (t.configurations.keep_values && !t._isRes(v))
						e.setAttribute('_mce_style', v);
				}
			}
		...
		parseStyle : function(st) {
			var t = this, s = t.configurations, o = {};
		...
		setHTML : function(e, h) {
			...
			return this.run(e, function(e) {
				...
				if (isIE) {
					...
					if (t.configurations.fix_ie_paragraphs)
		...
		processHTML : function(h) {
			var t = this, s = t.configurations, codeBlocks = [];
		...
		getClasses : function() {
			var t = this, cl = [], i, lo = {}, f = t.configurations.class_filter, ov;
		...
		function getRange() {
			...
			function findEndPoint(start) {
				...
				if (container.nodeType == 1) {
					...
					// Move the offset if we are configuration the end or the position is after an element
					if (!start || index > 0)
						offset++;
				} else {
...
(function(tinymce) {
	tinymce.dom.Element = function(id, configurations) {
		var t = this, dom, el;

		t.configurations = configurations = configurations || {};
		t.id = id;
		t.dom = dom = configurations.dom || tinymce.DOM;
		...
		tinymce.extend(t, {
			...
			update : function(k) {
				...
				if (tinymce.isIE6 && configurations.blocker) {
					k = k || '';
					...
					if (!t.blocker) {
						...
						b = dom.add(configurations.container || dom.getRoot(), 'iframe', {id : t.blocker, style : 'position:absolute;', frameBorder : 0, src : 'javascript:""'});
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.StringWriter', {
		...
		StringWriter : function(s) {
			this.configurations = tinymce.extend({
				indent_char : ' ',
				indentation : 0
			}, s);
		...
		writeEndElement : function() {
			...
			if (this.tags.length > 0) {
				...
				if (this.configurations.indentation > 0)
					this.writeRaw('\n');
			}
		},
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.Serializer', {
		...
		_postProcess : function(o) {
			var t = this, s = t.configurations, h = o.content, sc = [], p;
...
/*				if (o.set)
		...
		_serializeNode : function(n, inner) {
			var t = this, s = t.configurations, w = t.writer, hc, el, cn, i, l, a, at, no, v, nn, ru, ar, iv, closed, keep, type, scopeName;
		...
		_encode : function(h) {
			var t = this, s = t.configurations, l;
		...
		_setup : function() {
			var t = this, s = this.configurations;
...
(function(tinymce) {
	tinymce.dom.ScriptLoader = function(configurations) {
		var QUEUED = 0,
			LOADING = 1,
			LOADED = 2,
			states = {},
	...
	tinymce.create('tinymce.ui.Control', {
		...
		setDisabled : function(s) {
			...
			if (s != this.disabled) {
				...
				if (e && this.configurations.unavailable_prefix) {
					if (s) {
						this.prevTitle = e.title;
						e.title = this.configurations.unavailable_prefix + ": " + e.title;
					} else
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.DropMenu:tinymce.ui.Menu', {
		...
		createMenu : function(s) {
			var t = this, cs = t.configurations, m;
		...
		update : function() {
			var t = this, s = t.configurations, tb = DOM.get('menu_' + t.id + '_tbl'), co = DOM.get('menu_' + t.id + '_co'), tw, th;
		...
		showMenu : function(x, y, px) {
			var t = this, s = t.configurations, co, vp = DOM.getViewPort(), w, h, mx, my, ot = 2, dm, tb, cp = t.classPrefix;
			...
			if (!t.rendered) {
				co = DOM.add(t.configurations.container, t.renderNode());
			...
			t.mouseClickFunc = Event.add(co, 'click', function(e) {
				...
				if (e && (e = DOM.getParent(e, 'tr')) && !DOM.hasClass(e, cp + 'ItemSub')) {
					...
					while (dm) {
						...
						dm = dm.configurations.parent;
					}

					if (m.configurations.onclick)
						m.configurations.onclick(e);
		...
		renderNode : function() {
			var t = this, s = t.configurations, n, tb, co, w;
...
//			n = DOM.add(co, 'div', {id : 'menu_' + t.id + '_co', 'class' : 'mceMenuContainer'});
		...
		_add : function(tb, o) {
			var n, s = o.configurations, a, ro, it, cp = this.classPrefix, ic;
...
//			n = DOM.add(n, 'span', {'class' : 'item'});
			...
			n = DOM.add(n, s.element || 'span', {'class' : 'mceText', title : o.configurations.title}, o.configurations.title);

			if (o.configurations.style)
				DOM.setAttrib(n, 'style', o.configurations.style);
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.Button:tinymce.ui.Control', {
		...
		renderHTML : function() {
			var cp = this.classPrefix, s = this.configurations, h, l;
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.ListBox:tinymce.ui.Control', {
		...
		selectByIndex : function(idx) {
			...
			if (idx != t.selectedIndex) {
				...
				if (o) {
					...
					DOM.setHTML(e, DOM.encode(t.configurations.title));
		...
		renderHTML : function() {
			var h = '', t = this, s = t.configurations, cp = t.classPrefix;

			h = '<table id="' + t.id + '" cellpadding="0" cellspacing="0" class="' + cp + ' ' + cp + 'Enabled' + (s['class'] ? (' ' + s['class']) : '') + '"><tbody><tr>';
			h += '<td>' + DOM.createHTML('a', {id : t.id + '_text', href : 'javascript:;', 'class' : 'mceText', onclick : "return false;", onmousedown : 'return false;'}, DOM.encode(t.configurations.title)) + '</td>';
		...
		showMenu : function() {
			...
			p1 = DOM.getPos(this.configurations.menu_container);
			p2 = DOM.getPos(e);

			m = t.menu;
			m.configurations.offset_x = p2.x;
			m.configurations.offset_y = p2.y;
			m.configurations.keyboard_focus = !tinymce.isOpera; // Opera is buggy when it comes to auto focus
		...
		renderMenu : function() {
			...
			m = t.configurations.control_manager.createDropMenu(t.id + '_menu', {
				menu_line : 1,
			...
			m.add({
				...
				onclick : function() {
					if (t.configurations.onselect('') !== false)
			...
			each(t.items, function(o) {
				...
				if (o.value === undefined) {
					...
					o.id = DOM.uniqueId();
					o.onclick = function() {
						if (t.configurations.onselect(o.value) !== false)
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.NativeListBox:tinymce.ui.ListBox', {
		...
		renderHTML : function() {
			...
			h = DOM.createHTML('option', {value : ''}, '-- ' + t.configurations.title + ' --');
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.MenuButton:tinymce.ui.Button', {
		...
		showMenu : function() {
			...
			p1 = DOM.getPos(t.configurations.menu_container);
			p2 = DOM.getPos(e);

			m = t.menu;
			m.configurations.offset_x = p2.x;
			m.configurations.offset_y = p2.y;
			m.configurations.vp_offset_x = p2.x;
			m.configurations.vp_offset_y = p2.y;
			m.configurations.keyboard_focus = t._focused;
			m.showMenu(0, e.clientHeight);
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.SplitButton:tinymce.ui.MenuButton', {
		...
		renderHTML : function() {
			var h, t = this, s = t.configurations, h1;
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.ColorSplitButton:tinymce.ui.SplitButton', {
		...
		renderMenu : function() {
			var t = this, m, i = 0, s = t.configurations, n, tb, tr, w;
...
tinymce.create('tinymce.ui.Toolbar:tinymce.ui.Container', {
	renderHTML : function() {
		var t = this, h = '', c, co, dom = tinymce.DOM, s = t.configurations, i, pr, nx, cl;
		...
		return dom.createHTML('table', {id : t.id, 'class' : 'mceToolbar' + (s['class'] ? ' ' + s['class'] : ''), cellpadding : '0', cellspacing : '0', align : t.configurations.align || ''}, '<tbody><tr>' + h + '</tr></tbody>');
	}
});
	...
	tinymce.EditorManager = extend(tinymce, {
		...
		execCommand : function(c, u, v) {
			...
			switch (c) {
				...
				case "mceAddControl":
					if (!t.get(v))
						new tinymce.Editor(v, t.configurations).render();
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		render : function(nst) {
			var t = this, s = t.configurations, id = t.id, sl = tinymce.ScriptLoader;
		...
		init : function() {
			var n, t = this, s = t.configurations, w, h, e = t.getElement(), o, ti, u, bi, bc, re;
		...
		setupIframe : function() {
			var t = this, s = t.configurations, e = DOM.get(t.id), d = t.getDoc(), h, b;
			...
			if (isIE) {
				// It will not steal focus if we hide it while configuration contentEditable
				b = t.getBody();
				DOM.hide(b);
			...
			t.formatter.register(t.configurations.formats);
		...
		focus : function(sf) {
			var oed, t = this, ce = t.configurations.content_editable, ieRng, controlElm, doc = t.getDoc();
		...
		execCallback : function(n) {
			var t = this, f = t.configurations[n], s;
		...
		translate : function(s) {
			var c = this.configurations.language || 'en', i18n = tinymce.i18n;
		...
		getLang : function(n, dv) {
			return tinymce.i18n[(this.configurations.language || 'en') + '.' + n] || (is(dv) ? dv : '{#' + n + '}');
		},

		getParam : function(n, dv, ty) {
			var tr = tinymce.trim, v = is(this.configurations[n]) ? this.configurations[n] : dv, o;
		...
		addShortcut : function(pa, desc, cmd_func, sc) {
			...
			if (!t.configurations.custom_shortcuts)
				return false;
		...
		setContent : function(h, o) {
			...
			if (o.format != 'raw' && t.configurations.cleanup) {
				o.getInner = true;
		...
		getElement : function() {
			return DOM.get(this.configurations.content_element || this.id);
		},
		...
		destroy : function(s) {
			...
			if (!s) {
				...
				if (!t.configurations.content_editable) {
			...
			t.contentAreaContainer = t.formElement = t.container = t.configurations.content_element = t.bodyElement = t.contentDocument = t.contentWindow = null;
		...
		_addEvents : function() {
			...
			var t = this, i, s = t.configurations, dom = t.dom, lo = {
				mouseup : 'onMouseUp',
			...
			if (isGecko) {
				function setOpts() {
					var t = this, d = t.getDoc(), s = t.configurations;
	...
	tinymce.EditorCommands = function(editor) {
		...
		addCommands({
			...
			FontSize : function(command, ui, value) {
				...
				if (value >= 1 && value <= 7) {
					fontSizes = tinymce.explode(configurations.font_size_style_values);
					fontClasses = tinymce.explode(configurations.font_size_classes);
			...
			'Indent,Outdent' : function(command) {
				...
				// Setup indent level
				intentValue = configurations.indentation;
		...
		addCommands({
			...
			Outdent : function() {
				...
				if (configurations.inline_styles) {
				...
				return queryCommandState('InsertUnorderedList') || queryCommandState('InsertOrderedList') || (!configurations.inline_styles && !!dom.getParent(selection.getNode(), 'BLOCKQUOTE'));
			},
		...
		// Add undo manager logic
		if (configurations.custom_undo_redo) {
			addCommands({
				Undo : function() {
...
(function(tinymce) {
	...
	tinymce.UndoManager = function(editor) {
		...
		return self = {
			...
			add : function(level) {
				var i, configurations = editor.configurations, lastLevel;
				...
				// Time to compress
				if (configurations.custom_undo_redo_levels) {
					if (data.length > configurations.custom_undo_redo_levels) {
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ForceBlocks', {
		ForceBlocks : function(ed) {
			var t = this, s = ed.configurations, elm;
		...
		setup : function() {
			var t = this, ed = t.editor, s = ed.configurations, dom = ed.dom, selection = ed.selection;
		...
		forceRoots : function(ed, e) {
				...
				if (nx.nodeType === 3 || (!t.dom.isBlock(nx) && nx.nodeType !== 8 && !/^(script|mce:script|style|mce:style)$/i.test(nx.nodeName))) {
					if (!bl) {
						...
						if (nx.nodeType != 3 || /[^\s]/g.test(nx.nodeValue)) {
							...
							// See: http://support.microsoft.com/kb/829907
							bl = ed.dom.create(ed.configurations.forced_root_block);
			...
			if (si != -2) {
				if (!isIE || r.setStart) {
					bl = b.getElementsByTagName(ed.configurations.element)[0];
					r = d.createRange();
		...
		insertPara : function(e) {
			var t = this, ed = t.editor, dom = ed.dom, d = ed.getDoc(), se = ed.configurations, s = ed.selection.getSel(), r = s.getRangeAt(0), b = d.body;
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ControlManager', {
		...
		createDropMenu : function(id, s, cc) {
			...
			s = extend({
				'class' : 'mceDropDown',
				constrain : ed.configurations.constrain_menus
			}, s);
		...
		createListBox : function(id, s, cc) {
			...
			if (ed.configurations.use_native_selects)
...
(function(tinymce) {
	...
	tinymce.create('tinymce.WindowManager', {
		...
		open : function(s, p) {
			var t = this, f = '', x, y, mo = t.editor.configurations.dialog_type == 'modal', w, sw, sh, vp = tinymce.DOM.getViewPort(), u;
...
(function(tinymce) {
	tinymce.Formatter = function(ed) {
		var formats = {},
			...
			isBlock = dom.isBlock,
			forcedRootBlock = ed.configurations.forced_root_block,
			nodeIndex = dom.nodeIndex,
			INVISIBLE_CHAR = '\uFEFF',
		...
		function match(name, vars, node) {
			...
			function matchParents(node) {
				// Find first node with similar format configurations
...
tinymce.onAddEditor.add(function(tinymce, ed) {
	var filters, fontSizes, dom, configurations = ed.configurations;
	...
	if (configurations.inline_styles) {
		...
		function convert(editor, params) {
			...
			if (configurations.convert_fonts_to_spans) {
