	...
	tinymce.create('tinymce.plugins.PastePlugin', {
		init : function(ed, url) {
			...
			function grabContent(e) {
				...
				if (tinymce.isIE) {
					...
					// to IE security configurations so we pass the junk though better than nothing right
					if (n.innerHTML === '\uFEFF') {
		...
		_insertPlainText : function(ed, dom, h) {
			...
			if ((typeof(h) === "string") && (h.length > 0)) {
				if (!entities)
					entities = ("34,quot,38,amp,39,apos,60,lt,62,gt," + ed.serializer.configurations.entities).split(",");
